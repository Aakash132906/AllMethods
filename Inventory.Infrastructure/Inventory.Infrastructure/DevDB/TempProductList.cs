using System;
using System.Collections.Generic;

namespace PersonalWork.Infrastructure.DevDB;

public partial class TempProductList
{
    public long Fid { get; set; }

    public int? ProductId { get; set; }

    public int BatchId { get; set; }

    public string? Name { get; set; }

    public string? Enname { get; set; }

    public string? RuName { get; set; }

    public string? EsName { get; set; }

    public string? Imgurl { get; set; }

    public string? Price { get; set; }

    public string? SmallImagurl { get; set; }

    public string? AddCount { get; set; }

    public string? BoxCount { get; set; }

    public string? TotalPcs { get; set; }

    public string? TotalGw { get; set; }

    public string? TotalCbm { get; set; }

    public string? TotalNw { get; set; }

    public string? ProductCode { get; set; }

    public string? ProductModelId { get; set; }

    public string? ProductModelName { get; set; }

    public string? ProductTypeName { get; set; }

    public string? ProductTypeId { get; set; }

    public string? BoxSize01 { get; set; }

    public string? BoxSize02 { get; set; }

    public string? BoxSize03 { get; set; }

    public string? BoxSizeSum { get; set; }

    public string? BoxSize { get; set; }

    public string? SampleSize01 { get; set; }

    public string? SampleSize02 { get; set; }

    public string? SampleSize03 { get; set; }

    public string? SampleSize { get; set; }

    public string? PakingTypeId { get; set; }

    public string? PakingTypeName { get; set; }

    public string? Nw { get; set; }

    public string? Gw { get; set; }

    public string? Pcs { get; set; }

    public string? Cntcbm { get; set; }

    public string? Ceritficate { get; set; }

    public string? IsZh { get; set; }

    public string? IsEn { get; set; }

    public string? IsRu { get; set; }

    public string? IsEs { get; set; }

    public string? Bhdes { get; set; }

    public string? LastUpdate { get; set; }

    public string? FacPno { get; set; }

    public string? ShowPrice { get; set; }

    public string? FactoryCode { get; set; }

    public string? FactoryName { get; set; }

    public string? DetailDescription { get; set; }

    public string? ProductState { get; set; }

    public string? FacCode { get; set; }

    public string? PurchasePrice { get; set; }

    public string? MeterialArea { get; set; }

    public string? VideoUrl { get; set; }

    public string? ProductVideo { get; set; }

    public string? Id { get; set; }

    public string? CreateDate { get; set; }

    public string? Weight { get; set; }

    public string? Isactive { get; set; }
}
